...
class Task:
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            angle = randint(0,360)
            donnie_x = cos(radians(angle))*distance
            donnie_y = sin(radians(angle))*distance
        else:
            donnie_x = int(input("Enter Donnie's x: "))
            donnie_y = int(input("Enter Donnie's y: "))
        self.donnie = Home(donnie_x,donnie_y,32,32,IMG_HOME,"Donnie")
        stage.add_object(self.donnie)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Donnie's home is at ({self.donnie.x:.2f},{self.donnie.y:.2f})")
